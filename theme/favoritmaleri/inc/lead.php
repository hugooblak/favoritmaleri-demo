 ...
 * or photo. They call fa_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Favorit Måleri AB.
 *
 ...
 * @return string
 */
function fa_sample_note() {
	return fa_lead( 'ar_riktig' ) ? '' : ' <span class="fa-sample">(platshållare)</span>';
}
 ...
 * @return string File name for fa_lead_img().
 */
function fa_lead_bild( $i ) {
	$bilder = (array) fa_lead( 'bilder', array() );
 ...
 * @return string
 */
function fa_ring_label() {
	$namn = fa_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, fa_lead( 'telefon' ) ) : 'Ring ' . fa_lead( 'telefon' );
}
