<?php
/**
 * Favorit Måleri AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Favorit Måleri AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through fa_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'favoritmaleri', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'favoritmaleri' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'favoritmaleri' ) ) );

		register_block_pattern_category( 'favoritmaleri-sections', array( 'label' => __( 'Favorit Måleri AB sections', 'favoritmaleri' ) ) );
		register_block_pattern_category( 'favoritmaleri-pages', array( 'label' => __( 'Favorit Måleri AB page layouts', 'favoritmaleri' ) ) );
	}
);
