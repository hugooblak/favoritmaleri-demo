...
add_filter(
	...
	function ( $exporters ) {
		$exporters['favoritmaleri-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'favoritmaleri-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'favoritmaleri-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'favoritmaleri-leads',
						'group_label' => __( 'Quote requests', 'favoritmaleri-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['favoritmaleri-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'favoritmaleri-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
